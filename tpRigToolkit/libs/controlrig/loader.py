...
def init(do_reload=False, dev=False):
    ...
    class tpNameIt(importer.Importer, object):
        def __init__(self, *args, **kwargs):
            super(tpNameIt, self).__init__(module_name='tpRigToolkit.libs.controlrig', *args, **kwargs)
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        mod_dir = tpRigToolkit.libs.controlrig.__path__[0]
                    except Exception:
                        return None

            return mod_dir

    controlrig_importer = importer.init_importer(importer_class=tpNameIt, do_reload=False)

    register.register_class('logger', logger)

    controlrig_importer.import_modules()
    controlrig_importer.import_packages(only_packages=True)
    if do_reload:
        controlrig_importer.reload_all()
...
def create_logger_directory():
    ...
    nameit_importer = os.path.normpath(os.path.join(os.path.expanduser('~'), 'tpRigToolkit-libs-controlrig', 'logs'))
